...
async function login(email, password) {
  ...
  const passwordProvided = submittedPassword.length > 0;

  if (!emailMatches || !passwordProvided) {
    const comparisonPassword = passwordProvided ? submittedPassword : 'not-a-valid-password';
    await bcrypt.compare(comparisonPassword, adminPasswordHash);
    return false;
  }

  return bcrypt.compare(submittedPassword, adminPasswordHash);
}
